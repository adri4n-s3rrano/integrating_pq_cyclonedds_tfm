...
//Name: Adrián Serrano Navarro (100429115)
...
typedef struct HandshakeInfo
{
  ...
  HashValue_t hash_c1;
  HashValue_t hash_c2;
  EVP_PKEY *ldh; //Sustitute
  EVP_PKEY *rdh; //Sustitute
  uint8_t *kem_public; 
...
static LocalIdentityInfo *local_identity_info_new(DDS_Security_DomainId domainId, X509 *identityCert, X509 *identityCa, EVP_PKEY *privateKey, X509_CRL *crl, const DDS_Security_GUID_t *candidate_participant_guid, const DDS_Security_GUID_t *adjusted_participant_guid)
{
  ...
  assert(sizeof(DDS_Security_IdentityHandle) == 8); //Sustitute although i think it corresponds to the number of arguments. 
  ...
  identity->dsignAlgoKind = get_authentication_algo_kind(identityCert, PLUGIN_DEBUG); //Sustitute
  identity->kagreeAlgoKind = AUTH_ALGO_KIND_EC_PRIME256V1; //Sustitute 
...
static RemoteIdentityInfo *remote_identity_info_new(const DDS_Security_GUID_t *guid, const DDS_Security_IdentityToken *remote_identity_token)
{
  ...
  identity->kagreeAlgoKind = AUTH_ALGO_KIND_UNKNOWN; //Sustitute value pre-encapsulate 
...
DDS_Security_ValidationResult_t begin_handshake_reply(dds_security_authentication *instance, DDS_Security_HandshakeHandle *handshake_handle,
  ...
  if (!handshake->kem_ciphertext || !localIdent->kem_secret){
    ...
    printf("\nkem public token value\n");
    for (size_t i = 0; i < binary_property_kem_public->value._length; ++i) {
        printf("%02X ", binary_property_kem_public->value._buffer[i]); // Imprimir cada byte en hexadecimal
    }
    ...
    for (size_t i = 0; i < OQS_KEM_kyber_768_length_ciphertext; i++) {
        printf("%u ", kem_ciphertext[i]); // Imprime cada elemento del array
    }
